...
Creating a dedicated wheel user and removing wheel from your primary user helps prevent certain privilege escalation attack vectors and password sniffing. To do this, you will log in as admin to perform the final step of removing the user accounts\'s wheel privileges, which depends on having acess to your admin and it functioning correctly first. You don\'t need to log in using your wheel(admin) user to use it for privileged operations. When logged in as your non-wheel user, Polkit will prompt you to authenticate as your wheel user as needed, or when requested by calling <code>run0</code>.
