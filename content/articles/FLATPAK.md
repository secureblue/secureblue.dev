...
Note that this will not only undo the the `ujust flatpak-permissions-lockdown` command but also any other global overrides (individual app overrides will not be affected). This also affects flatpak's [Hardened_Malloc](https://github.com/GrapheneOS/hardened_malloc) integration (which is set by default), so you should run this afterwards:
